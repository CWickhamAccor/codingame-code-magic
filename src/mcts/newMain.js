...
const curve = [
    { low: 0, high: 2, count: 10 },
    { low: 3, high: 5, count: 10 },
    { low: 6, high: 8, count: 6 },
    { low: 9, high: 12, count: 4 },
];
...
function attack(myBoard, source, target) {
    ...
    } else if (target.ward && source.power > 0) {
        target.ward = false;
    } else {
...
function getPossibleAttacks(game) {
    ...
    if (memoHelper.actions.attacks.has(key)) {
        // debug('(attacks) memo used !');
        return copy(memoHelper.actions.attacks.get(key));
    }
    const attacks = [];
    const oppGuards = oppBoard.filter(crea => crea.guard);
    myBoard.filter(crea => !crea.sick && !crea.attacked && crea.power > 0)
        .forEach((crea) => {
    ...
    memoHelper.actions.attacks.set(key, copy(attacks));
    return attacks;
}
...
function draft({ player }, cards) {
    ...
    cards.forEach((card) => {
        if (!Object.keys(scale).includes(card.number.toString())) {
            ...
            card.score = scale[card.number];
            const curveElem = curve.find(elem => card.ccm >= elem.low && card.ccm <= elem.high);
            card.score += curveElem.count * player.deck / 30;
            debug('score : ', card.score);
        }
    });
    cards.sort((card1, card2) => card2.score - card1.score);
    const curvePicked = curve.find(elem => cards[0].ccm >= elem.low && cards[0].ccm <= elem.high);
    curvePicked.count -= 1;
